...
​	数组必须事先定义固定的长度，不能适应数据动态地增减的情况。从**栈**中分配空间, 对于程序方便快速,但是自由度小。
...
链表动态地进行存储分配，可以适应数据动态地增减的情况。从**堆**中分配空间, 自由度大但是申请管理比较麻烦。
...
* 类加载机制（编译后的class文件怎么加载到JVM呢？-> 类加载机制）

  ![1582025765643](assert/1582025765643.png)

首先jvm 自己启动，即渡己rt----runtime
...
是先存在方法区，然后运行是先去解释执行，main 入口

方法区-堆-方法区：又叫静态存储区，存放class文件和静态数据，线程共享

-类中的成员变量存放在堆区；

栈是以线程为单位的；创建线程就给存储的栈；

---方法中的局部变量，存放在栈区

堆是共有的，栈是私有的；

具体解释执行通过本地方法栈---调用本地（Windows）方法pci；

程序计数器记录不同线程栈执行到哪了哪一行了
...
创建的对象 存在伊甸园，1 伊甸园满了后出发GC ，被用过一次不再用的垃圾回收，将一直被需要和调用的对象留着，放幸存者一区； 2 再存 伊甸园又满了 再GC ，将伊甸园幸存的和s1区的都放在S2 ； 3 再存伊甸园又满GC，再把伊甸园和S2 中幸存的挪S1；来回S1 S2这样复制，每复制一次，对象来回复制次数增加；15次后移动到老年代；当老年代满后触发FullGC，清空堆中所有，此时所有的JVM操作是暂停的；---JVM调优 避免FullGC；

年轻代主要用的是复制算法；持久带中常量一般是很大的对象（运行初期创建size10000的list）;1.7经常用的是CMS收集器；

1.7在堆内存有限的情况下，不能完美的利用空间，1.8 升级；性能和内存利用率有大的提升，

1.8把内存化为同样大小的region，可以做不同用法，根据所需动态改变；老年代空间大可以减少FullGC，对象满后，用G1GC；
...
* @Controller 		控制层
* @RestController           控制层  提供json数据格式
* @Service            
* @Autowired         spring依赖注入
* @Resource              java自带依赖注入
* @Configuration                 配置类
* @Bean           bean对象
* @Value             配置类中定义了很多key=value   通过这样把配置中的值引用过来，直接给当前属性赋值
* @Scheduled          定时任务
* @ExceptionHandler              java中异常处理的注解
...
![1599138781244](assert\1599138781244.png)
